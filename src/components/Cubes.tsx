...
const Cubes: React.FC<CubesProps> = ({
    ...
    const colGap = typeof cellGap === 'number' ? `${cellGap}px` : (cellGap && 'col' in cellGap && cellGap.col !== undefined) ? `${cellGap.col}px` : '5%';
    const rowGap = typeof cellGap === 'number' ? `${cellGap}px` : (cellGap && 'row' in cellGap && cellGap.row !== undefined) ? `${cellGap.row}px` : '5%';
    ...
    const tiltAt = useCallback(
        (rowCenter: number, colCenter: number) => {
            ...
            sceneRef.current.querySelectorAll('.cube').forEach((cubeElement) => {
